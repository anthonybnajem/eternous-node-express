...
Set `LOG_REPORT_ENABLED=false` in your `.env` file or comment out the `startLogReportScheduler()` call in `src/index.ts`.
...
**Configuration:** See `migrate-mongo-config.ts` and `migrations/` directory.
